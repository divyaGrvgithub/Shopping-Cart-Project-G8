...
const { createProductJoi, updateProductJoi } = require("../validation/validator");
...
const createProduct = async (req, res) => {
  try {
    ...
    if (data.availableSizes) {
      ...
      for (let i of reqSize) {
        if (!enumValue.includes(i)) {
          return res.status(400).send({ status: false, message: `Sizes can be only from "S", "XS","M","X", "L","XXL", "XL" these`, })
        }
      }
    ...
    const existingTitle = await productModel.findOne({ $and: [{ title: data.title, isDeleted: false }] });
    if (existingTitle)
      return res.status(400).send({ status: false, message: "title already exist please enter another one" });

    //data creation
    const createProduct = await productModel.create(data);

    return res
      .status(201).send({ status: true, message: "Success", data: createProduct });
  } catch (error) {
...
const getAllProduct = async (req, res) => {
  try {
    ...
    if (Object.keys(rest).length > 0)
      return res.status(400).send({ status: false, message: "You can filter only by size, name, priceGreaterThan, priceLessThan, priceSort " });
    ...
    if (priceSort) {
      if (!(priceSort == "1" || priceSort == "-1"))
        return res.status(400).send({ status: false, message: "value priceSort can either be 1 or -1" });
  ...
  try {
    ...
    if (!products)
      return res.status(404).send({
        status: true, message: "This product is deleted or not found"
      });
    else {
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (!mongoose.isValidObjectId(productId))
      return res.status(400).send({ status: false, message: "productId in param is not a valid product id", });
    ...
    if (titleExist)
      return res.status(400).send({ status: false, message: "title already in use please use another one" });
    ...
    if (files && files.length > 0) {
      if (
        ...
        return res
          .status(400).send({ status: false, message: "you can upload only image file" });
    ...
    if (Object.keys(data).length == 0)
      return res.status(400).send({ status: false, message: "please enter atleast one field in order to update it" });
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (!productId)
      return res.status(400).send({ status: false, message: "please enter productID first" });
    ...
    if (!deleteProduct)
      return res.status(404).send({ status: true, message: "This product is deleted or not found" });
    else {
