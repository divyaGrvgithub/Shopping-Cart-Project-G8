...
const login = async (req,res) =>{
    try{ 
     ...
     if(Object.keys(data).length==0) return res.status(400).send({status:false, message:"please enter some data"})
     try {
         const valid = await loginjoi.validateAsync(data)
     } catch (error) {
         return res.status(400).send({status:false, error: error.message})
     }
     let findUser = await UserModel.findOne({email:email.trim().toLowerCase()})
     if(!findUser) return res.status(404).send({status:false,message:"Credentials do not match"})
 ...
 module.exports = {login,getUserProfile,createUser}
