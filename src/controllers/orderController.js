  ...
  try {
    ...
    if (status) {
      if (cancellable == false && status == "cancelled")
        return res.status(400).send({ status: false, message: "if cancel is not allowed the status cannot be cancelled" });

      if (!["pending", "completed", "cancelled"].includes(status.toLowerCase()))
        return res.status(400).send({ status: false, message: `the status can be from ["pending", "completed", "cancelled"]` })
    }
    ...
    await cartModel.findOneAndUpdate({ _id: cartId }, { $set: { items: [], totalItems: 0, totalPrice: 0 } }, { new: true });

    newOrderCreate = newOrderCreate.toObject()
    delete newOrderCreate.isDeleted
    return res.status(201).send({ status: true, message: "Success", data: newOrderCreate });
  } catch (error) {
